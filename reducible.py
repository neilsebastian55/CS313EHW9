...
def insert_word(s, hash_table):
    ...
    hash_index = hash_word(s, hash_size)
    step = step_size(s)

    while hash_table[hash_index] != "":
        if hash_table[hash_index] == s:
            return
        hash_index = (hash_index + step) % hash_size
...
def get_longest_words(string_list):
    ...
    if not string_list:
        return []
    max_word_length = max(len(word) for word in string_list)
    return [word for word in string_list if len(word) == max_word_length]
...
def main():
    ...
    prime_size = next(n for n in range(list_size * 2 + 1, list_size * 4) if is_prime(n))

    hash_table = [""] * prime_size
    for word in word_list:
        insert_word(word, hash_table)

    memo_size = next(n for n in range(int(0.2 * list_size), list_size) if is_prime(n))
    hash_memo = [""] * memo_size
